...
function rerankSetsWithMethodPerformance(sets, methodPerformance) {
    if (sets.length === 0)
        return sets;
    const statsByMethod = new Map((methodPerformance || []).map((snapshot) => [snapshot.name, snapshot]));
    const methodAverages = (methodPerformance || []).map((snapshot) => snapshot.avgOverlap);
    const minMethodAvg = methodAverages.length > 0 ? Math.min(...methodAverages) : 0;
    const maxMethodAvg = methodAverages.length > 0 ? Math.max(...methodAverages) : 1;
    ...
    const bestLift = Math.max(...sets.map((set) => (Number.isFinite(set.relativeLift) ? set.relativeLift : 0)), 1e-9);
    const numberSupport = new Map();
    for (let idx = 0; idx < sets.length; idx++) {
        const set = sets[idx];
        const rankWeight = 1 / (idx + 1);
        const liftWeight = clampNumber((Number.isFinite(set.relativeLift) ? set.relativeLift : 0) / bestLift, 0, 1.25);
    ...
    const supportScores = sets.map((set) => set.numbers.reduce((sum, n) => sum + (numberSupport.get(n) || 0), 0) / analysis_1.K);
...
onProgress, runtimeBudgets = calibrateRuntimeBudgets(draws.length, {
    ...
    const recordMethodOverlap = (method, overlap) => {
    ...
    const overlapPrior = processedTestSize > 0 ? top6TotalOverlap / processedTestSize : (analysis_1.K + 1) / N;
    const methodShrinkage = Math.max(8, Math.min(32, Math.round(processedTestSize * 0.12)));
    const methodPerformance = Array.from(methodPerformanceMap.entries())
        .map(([name, stats]) => {
        const shrunkAverage = (stats.overlapTotal + overlapPrior * methodShrinkage) /
...
function buildFinalPredictionArtifacts(options) {
    ...
    const rankedSets = prependConsensusSet(sets, finalScores, options.diagnostics.poolSize, learnedDiagnostics);
    const finalizedSets = rerankSetsWithMethodPerformance(rankedSets, options.backtest.methodPerformance).slice(0, 10);
